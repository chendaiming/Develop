	...
	var selectcomponent = vue.extend({
		...
		watch:{
			'id':function(){
				if((this.id && this.id !='') || this.id=='0'){
					...
					if(this.list && this.isArray(this.list)){
						this.findName();
					}else{
						this.name="";
					}
		...
		methods : {
			isArray:function(arry){
				return {}.toString.call(arry) === '[object Array]';
			},
			findName:function(){
				for(var i=0,j=this.datas.length;i<j;i++){
				...
				if(me.code){
					...
					db.query({
						...
						success: function (data) {
							if(me.list && me.isArray(me.list)){
								me.datas=me.list;
								me.datas = [].concat(me.datas,data);
							}else{
								me.datas = data;
							}

...
define(function(require){
	...
	var bootstrap = require('bootstrap');
       ...
       //加载外来车辆在监情况 
        db.query({
    		request:{
    			sqlId:'select_foreign_car_inout_count',
    ...
    db.query({
		request:{
			sqlId:'select_foreign_car_inout_count_groupbytime',
			whereId:'0',
			orderId:'0',
			params:[login.cusNumber]
		},
     ...
     * 注册点击事件
     */
    tableBar.on('click',function(param){loadCarInfo(param.data.name,param.name);});
	 ...
	 */
	 gotoPanel = function(obj){
	    ...
	    db.query({
	    	request:{
		    	sqlId:'select_foreign_car_inout_infobyid',
		    	whereId:'0',
		    	orderId:'0',
	  ...
	  * 加载外来车辆信息
	  */
	 function loadCarInfo(value,name){
		 var wid='0',param=[];
		 switch(value){
		 case '在监':
			 param = [login.cusNumber,1];
			 model.viewtext = '在监车辆信息';
			 break;
		 case '离监':
			 param = [login.cusNumber,2];
			 model.viewtext = '离监车辆信息';
			 break;
		 case '待进':
			 param = [login.cusNumber];
			 wid = 1;
			 model.viewtext = '待进车辆信息';
			 break;
		default:
			if(isNaN(value)) return;//传入的数值不为数字时,不做处理直接返回
			wid = 3;
			param = [login.cusNumber,value];
			model.viewtext=name+'点进监车辆信息';
			break;
		 }
		 
	    db.query({
	    	request:{
		    	sqlId:'select_foreign_car_base_info',
		    	whereId:wid,
		    	orderId:'0',
		    	params:param
	       },
	       success:function(data){
	    		model.carinfo = [];
	    		if(!data || data.length == 0) return;
	    		model.carinfo = model.carinfo.concat(data);
	    		$('#infoUl').show();
	    	},
	    	error:function(errorCode, errorMsg){
				tip.alert(errorCode + ":" + errorMsg);

...
define(function(require) {
    ...
    (function(exports) {
        ...
        var buildPolygonNeighbours = function(polygon, navigationMesh) {
            ...
            for (var i = 0, len = navigationMesh.polygons.length; i < len; i++) {
                ...
                if (polygon.centroid.distanceToSquared(navigationMesh.polygons[i].centroid) > searchRadius * searchRadius) continue;
        ...
        var distanceToSquared2D = function(a, b) {
            var dx = a.x - b.x;
            var dz = a.z - b.z;
            return dx * dx + dz * dz;
        }
        ...
        function isVectorInPolygon(vector, polygon, vertices) {
            ...
            //     return true;
            // }
            if (isPointInPoly(polygonVertices, vector)) {
                return true;
            }
         ...
         * 判断是否在多边形内
         */
        function _isInPoly(vector, polygon, vertices) {
            var center = polygon.centroid;
            if (center.y > vector.y) { //查询点在center下方         
                return false;
            }
            var polygonVertices = [];
            _.each(polygon.vertexIds, function(vId) {
        ...
        Channel.prototype.stringPull = function() {
            ...
            // console.log('portalApex', portalApex);
        ...
        var searchRadius = 0;
        _.extend(exports, {
            isInPoly: function(zone, v) {
                var polygons = zoneNodes[zone].groups[0]; //分组为0也只能0
                var vertices = zoneNodes[zone].vertices;
                var b = false;
                for (var index = 0, l = polygons.length; index < l; index++) {
                    let p = polygons[index];
                    b = _isInPoly(v, p, vertices);
                    if (b === true) {
                        break;
                    }
                }
            ...
            buildNodes: function(geometry, radius) {
                searchRadius = radius || 50000;
            ...
            setZoneData: function(zone, data, radius) {
                zoneNodes[zone] = data;
                zoneNodesRadius[zone] = radius;
            },
            getGroup: function(zone, position) {
                ...
                var distance = Math.pow(zoneNodesRadius[zone], 2);
                ...
                //console.log(zoneNodes[zone].groups[closestNodeGroup]);
            ...
            getRandomPoint: function(zone, group, nearPosition, nearRange) {
                ...
                _.each(polygons, function(p) {
                    if (isVectorInPolygon(nearPosition, p, vertices) === true) { //确保找到当前点所在的网格中
                        currPolygon = p;
                    }
                    if (nearPosition && nearRange) {
                        if (distanceToSquared2D(nearPosition, p.centroid) <= nearRange * nearRange) {
                            candidates.push(p);
                        }
                    }
                });
            ...
            findPath: function(startPosition, targetPosition, zone, group) {
                ...
                distance = Math.pow(zoneNodesRadius[zone], 2);
                ...
                _.each(allNodes, function(node) {
                    ...
                    // }else
                    // {
                    //     console.log('max:'+measuredDistance);
                    // }
                    if (measuredDistance < distance &&
                ...
                if (!closestNode || !farthestNode) {
                    ...
                    // console.log('farthestNode:', farthestNode);
                    return null;
                }
    ...
    })(typeof exports === 'undefined' ? Patrol : exports);
    return Patrol;
});
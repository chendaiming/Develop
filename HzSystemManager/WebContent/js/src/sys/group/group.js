define(['jquery','vue','frm/hz.db','ztree','frm/model','frm/message','frm/treeUtil'],function($,tpl,db,treeSelect,modelUtil,tip,util){
			...
			if(model.group['agd_id']!=''){//更新
	 			db.updateByParamKey({
	 				...
	 				success:function(){
			 			...
			 			modelUtil.clear(model.group,{'pname':'角色分类','agd_type':'','agd_parent_id':'-1'});
			 			tip.alert("更新成功");
			 			initGroupTree();
	 				},
	 				error:function(a,b){
	...
	function validate(){
		var isError = false;
		$.each(model.treeData, function(idx, item) {
					if (item.agd_parent_id == model.group['agd_parent_id'] && item.agd_id != model.group['agd_id']) {
						if (item.agd_name == model.group['agd_name']) {
							isError = true;
							return false;
						}
					}
		...
		return !model.group['agd_name'].length&&!tip.alert("请输入角色组名称")||
				model.group['agd_name'].length>26&&!tip.alert("角色组名称过长，请输入26个以内字符")||
			   !model.group['agd_key'].length&&!tip.alert("请输入角色分类标识")||
			    model.group['agd_key'].length>26&&!tip.alert("角色分类标识过长，请输入26个以内字符");
 	}
});

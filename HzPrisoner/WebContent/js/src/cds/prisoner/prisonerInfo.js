...
//	})
	...
	if(rfid){	//通过地图房间RFID面板点击查看
		...
		var flag = sessionStorage.getItem("cdm-title-prisoner-flag");
   ...
   * 监狱节点和监区节点统计人数
   */
  function formatParentNode(data){
	  //获取有子节点的节点(即type为0的节点)
	  var nodes = treeContainer.getNodesByParam('type',0);
	  //遍历这些父节点
	  nodes.forEach(function(item,index){
			if(item.id != login.cusNumber){
				//获取对应ID的节点
				var node =  treeContainer.getNodeByParam('id',item.id);  
				//获取其子节点
				var childNode = treeContainer.getNodesByParam('pid',item.id); 
				//更新节点文本
				node.name = node.name + '(' + childNode.length + ')'; 
				treeContainer.updateNode(node);
			}else if(item.id == login.cusNumber){
				var node = treeContainer.getNodeByParam('id',item.id); 
				//除父节点外所有节点的length
				var total = data.length - nodes.length; 
				//更新节点文本
				node.name = node.name + '(' + total + ')';
				treeContainer.updateNode(node);
			}
	  });
  }

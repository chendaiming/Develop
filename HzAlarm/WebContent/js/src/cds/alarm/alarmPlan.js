define(function(require){
	...
	// 报警设备的查询SQL
	var alarmDvcSql = {
		'1': 'select_camera_alarm_for_alarm_plan',				// 摄像机：智能分析
		'2': 'select_door_alarm_for_alarm_plan',				// 门禁
		'3': 'select_talkback_alarm_for_alarm_plan',			// 对讲机
		'4': 'select_broadcast_alarm_for_alarm_plan',			// 广播
		'6': 'select_network_alarm_for_alarm_plan',				// 网络报警器
		'7': 'select_power_network_alarm_for_alarm_plan',		// 高压电网
		'9': 'select_vibratingfiber_device_for_alarm_plan',		// 周界振动光纤
		'15': 'select_rfid_alarm_for_alarm_plan',				// RFID基站
		'17': 'select_wireless_alarm_master_for_alarm_plan'		// 无线报警主机
	};

	// 默认显示的报警设备树的索引
	var defaultShowAlarmDvc = [0,1,2,3,5];
...
//						}
	...
	var vm = new vue({
		...
		watch:{
			'checkedStatus': function () {
				var searchVal, treeId, node, newNodes, cNodePidMap, pNodeMap;
				...
				function filterParentNodes (pid) {
					for(var id in pNodeMap) {
						if (id == pid) {
							node = pNodeMap[id];
							delete pNodeMap[id];
							newNodes.push(node);
							filterParentNodes(node.pid);
						}
					}
				}
				...
				for(var type in this.zTreeMaps) {
						...
						for(var i = 0, I = treeNodes.length; i < I; i++) {
							...
							if (node.hasLinked) {
								if (node.hasLinked == searchVal) {
									newNodes.push(node);
									cNodePidMap[node.pid] || (cNodePidMap[node.pid] = true);
								}
							} else {
								pNodeMap[node.id] = node;
							}
						}

						for(var pid in cNodePidMap) {
							filterParentNodes(pid);
						}
					...
					$('#'+treeId).toggle(newNodes.length > 0);
				}
			},
	...
	function loadDeptTree () {
		var vm = new vue({
			...
			data: {
				params: {
					...
					'lhs_id': '',				// 分级处置编号
					'lhs_dvc_type': '',			// 报警设备类型
					'lhs_alertor_id': '',		// 报警器编号
					'lhs_start_time': '00:00:00',		// 开始时间
					'lhs_end_time': '23:59:59',			// 结束时间
					'lhs_level': '1',			// 报警等级
					'lhs_receive_dept_id': '',	// 接警部门
					'lhs_seq': '',				// 序号
					'lhs_crte_time': '',		// 创建时间
					'lhs_crte_user_id': '',		// 创建人
					'lhs_updt_time': '',		// 更新时间
					'lhs_updt_user_id': ''		// 更新人
				},
				receive: {
					id: user.deptId,
					name: user.deptName
		...
		db.query({
			'request': {
				sqlId: 'select_dptt_tree_for_alarm_plan',
				whereId: 0,
				params: [user.cusNumber, user.cusNumber]
			},
			'success': function (data) {
				// 接警部门
				tree.init('receiveDept', {
					'diyClass': 'stree',
					...
					'callback': {
						'onClick': function (event, treeId, treeNode) {
							...
							vm.receive.id = vm.params.lhs_receive_dept_id = treeNode.id;
							vm.receive.name = treeNode.name;
						}
					}
				}, data);
				...
				tree.init('noticeDept', {
					...
					'callback': {
						...
						'onCheck': function (event, treeId, treeNode) {
							event.stopPropagation();
							if (treeNode.checked) {
								vm.noticeMap[treeId] = treeNode;
								names.push(treeNode.name);
							} else {
								delete vm.noticeMap[treeId];
								names.splice(names.indexOf(treeNode.name), 1);
							}
							vm.noticeNames = names.join(',');
						}
					}
				}, data);
			},
